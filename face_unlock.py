...
def capture_face_temp():
    ...
    print("Capturing face... Press 's' to save. Press 'q' to cancel")
    face_encoding = None
    ...
    while True:
        ...
        if key == ord('s') and face_encoding is not None:
            ...
            print("TEMP_FACE_PATH:", TEMP_FACE_PATH)
            print("Encoding shape:", face_encoding.shape)
            np.save(TEMP_FACE_PATH, face_encoding)
            print("Temporary face encoding saved")
            video.release()
...
def move_temp_face_to_user(user_id):
    ...
    if os.path.exists(TEMP_FACE_PATH):
        ...
        print(f"Face file moved to {user_path}")
    else:
        print("No temp face file foudn to move")
...
'''
    ...
    for file in os.listdir(ENCODING_DIR):
        if file.endswith('.npy') and file.startswith("temp_") is False:
            user_id = file.split("_")[0]
